...
from core.order import Order, TestOrder
...
class Product:
    ...
    def _get_or_place_order(self, transaction_type, quantity, price=None, trigger_price=None):
        ...
        order_id = cache.get(f'{self.trading_symbol}{order_type}{KiteConnect.TRANSACTION_TYPE_BUY}')
        order_id = order_id.decode('utf-8') if order_id else None
        if order_id:
